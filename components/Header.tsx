...
interface HeaderProps {
  currentPage: 'home' | 'forum_ai' | 'forum_discussion' | 'account';
  setPage: (page: 'home' | 'forum_ai' | 'forum_discussion' | 'account') => void;
}
    ...
    <header className="bg-white shadow-md">
      <div className="container mx-auto max-w-4xl px-4 py-4 flex items-center justify-between">
        <div className="flex items-center">
          <img src={tappedinLogo} className="w-16 h-16" alt="TappedIn logo" />
          <div>
        ...
        <nav className="flex items-center space-x-2">
          ...
          <button
            onClick={() => setPage('forum_ai')}
            className={`${navLinkClasses} ${currentPage === 'forum_ai' ? activeClasses : inactiveClasses}`}
          >
            Verified Answers
          </button>

          <button
            onClick={() => setPage('forum_discussion')}
            className={`${navLinkClasses} ${currentPage === 'forum_discussion' ? activeClasses : inactiveClasses}`}
          >
            Discussions
          </button>
